<x-app-employee>
    ...
    <div class="container mt-3 content">
        <a href="{{ route('empl-leave.submit-leave') }}" class="btn bg-primary mb-3 px-3 py-2 text-white">Ajukan
            Cuti</a>
        <div class="row">
            <h4 class="mb-3">History Cuti</h4>
            @foreach ($leaveData as $index => $leave)
                <div class="col-md-12 mb-3">
                    <div class="card card-frame">
                        <div
                            class="card-header px-4 py-2 text-capitalize {{ $leave->status == 'pending' ? 'bg-gradient-secondary' : ($leave->status == 'approved' ? 'bg-gradient-success' : ($leave->status == 'rejected' ? 'bg-gradient-danger' : ($leave->status == 'cancelled' ? 'bg-gradient-primary' : ''))) }}  text-white d-flex justify-content-between border">
                            <div class="">
